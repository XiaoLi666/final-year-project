...
namespace GameLogic {
	public class ActionRaycasting : ActionBase {
		...
		public override bool Update(bool pause = false) {
			bool tutorial_pause = false;

			// Need to keep analysing the gesture in a loop
			m_gestureAnalyser.Analysis(ref tutorial_pause);
			...
			if (Physics.Raycast(m_ray, out m_hit, m_maxDistance)) {
				...
				m_gestureAnalyser.InitGestureCheck(m_hit, ref tutorial_pause);
				m_animationAnalyser.Analysis(m_hit.collider.tag, tutorial_pause);

				// for refactor purpose
				m_tag = m_hit.collider.tag;
		...
		public ActionRaycasting(GameObject owner) : base(owner) {
			...
			// TODO: the design of this assignment is bad, need to polish
			m_gestureAnalyser.m_animationAnalyser = m_animationAnalyser;
			m_gestureAnalyser.m_playerComponent = m_component;
			m_gestureAnalyser.m_gameWorld = m_component.GameWorld;
		}
		...
		public void HandleRaycastPathNodeSeaweed () {
